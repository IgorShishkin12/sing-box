...
class Peripheral {
                        ...
                        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
                            Peripheral.this.asyncWithFuture(future, () -> {
                                ...
                                if (newState == BluetoothGatt.STATE_CONNECTED) {
                                    // Request a larger ATT MTU so the RNode firmware can send
                                    // full KISS frames in one BLE notification. The default
                                    // 23-byte MTU (20-byte payload) truncates large RF packets.
                                    // wakeCommand is called from onMtuChanged below.
                                    gatt.requestMtu(512);
                                }
                            });
                        }
    ...
    private static abstract class CommandCallback extends BluetoothGattCallback {
        ...
        @Override
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            // Silently ignore: MTU changes are not expected during most commands.
        }
    }
}
