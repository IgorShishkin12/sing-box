...
// trust_store.go — energy-persistent peer authentication cache.
//
...
type TrustStore struct {
	path string
	mu   sync.RWMutex
	m    map[string]trustEntry // peerHash → entry
}
...
// HMAC-SHA256 ensures the token cannot be used to recover the password, and
// automatically invalidates when the password changes.
